...
void cargar_lista_mesas( Lista* ListaEspera ){
    FILE* LISTAMESAS = fopen("LISTAMESAS.csv","r");
    int numero;

    char linea[1024];
    while( fgets(linea, 1023, LISTAMESAS) != NULL ){
...
void actualizar_lista_mesas( Lista* ListaEspera ){
    FILE* LISTAMESAS = fopen("LISTAMESAS.csv", "w");

    Mesa* mesa = first_L(ListaEspera);
    while(mesa != NULL){
        fprintf(LISTAMESAS,"%d,%c", mesa->numero,34);

        NombreTipos* platillo = first_L(mesa->listaPlatillos);
        while(platillo != NULL){
            fprintf(LISTAMESAS, "%s", platillo->nombre);

            platillo = next_L(mesa->listaPlatillos);
            if(platillo != NULL){
                fprintf(LISTAMESAS, ",");
            }else{
                fprintf(LISTAMESAS, "%c",34);
            }
        }
